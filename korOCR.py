...
def process_and_write_results(ocr, img_or_path, write_to): #paddle only folders
    #t0 = time.time()
    #text = mocr(img_or_path) #########
    #
    result = ocr.predict(input="korOCRcache.png")
    for res in result:  
        text=res['rec_texts']
        text=" ".join(text)
        print(text)
...
def run(
    ...
    ocr = PaddleOCR(
        ...
        text_recognition_model_name="korean_PP-OCRv5_mobile_rec", #НЕ МЕНЯТЬ
        ...
        while True:
            ...
            try:
                ...
                if isinstance(img, Image.Image) and not are_images_identical(img, old_img):
                    ##############
                    img.save("korOCRcache.png")
                    #из клипборда сохранить в картинку для паддле
                    process_and_write_results(ocr, img, write_to)
